...
def render_ui():
    ...
    if uploaded_file is not None:
        ...
        st.write(f"Debug: Video path set to {video_path}")
...
def apply_suggested_settings(current_settings, suggestions):
...
def display_statistics_and_management(stats, output_folder):
...
def display_visualizations(stats, saved_frames, filter_settings):
    ...
    if "processing_parameters" in stats and any(stats[key]["Avg"] is not None for key in stats if key != "processing_parameters"):
        try:
            ...
            st.error("Please check the console for more details and report this issue.")
    else:
        st.info("No data available for visualization. This may be because no frames met the criteria.")

def display_extracted_frames(saved_frames):
    if saved_frames:
...
def display_filter_analysis(stats, filter_settings):
